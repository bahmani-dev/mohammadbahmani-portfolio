...
const Button = ({ text, type }: Props) => {
  return (
    ...
    >
      ...
      <span className="relative transition-colors duration-300 delay-200 group-hover:text-white ease dark:text-white max-md:text-sm font-bold pr-5">
        {text}
      </span>

      <div className=" w-[48px] h-[48px] text-white dark:text-black relative z-10 rounded-full bg-yellow-500 flex items-center justify-center">
        <FaArrowRight />
      </div>
    </button>
  );
